...
int main( int argc, char* argv[] )
{
    ...
    cudaError_t s = cudaMalloc((void**) &d_reg_max, n);
    s = cudaMemset(d_reg_max, 0 ,n);
    if(s != cudaSuccess)
	printf("Malloc Error after memSet");
	
    s = cudaMemset(d_gl_max, 0 ,n);
    if(s != cudaSuccess)
	printf("Malloc Error after memSet for gl_max");
